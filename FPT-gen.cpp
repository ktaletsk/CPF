...
void fpt(int n, double xs, double xa, double dt, Ran *eran, ofstream& file){
...
//     delete[] p;
    ...
    do{
        ...
        //reflecting boundary
        if (x[0] > xa) x[0] = 2 * xa - x[0];
        ncount++;

        if (x[0]<milestone){
            file << dt * ncount << "\t" << flush;
//             cout << dt * ncount << "\t" << flush;
            milestone -= milestone_step;
        }
    } while (x[0] > xs);
...
{
    std::string fpt_file_name{"fpt"};
    int jobid = 0;
    if (narg == 2) {
        jobid = atoi(arg[1]);
        fpt_file_name += "_";
        fpt_file_name += std::to_string(jobid);
    }
    fpt_file_name += ".dat";
    ofstream fpt_file;
    ...
    for (int i=0; i<25; ++i){
        Ran eran(25*jobid+i);
        fpt(1, 0.0, 1.0, 0.0001, &eran, fpt_file);
    }
    fpt_file.close();
